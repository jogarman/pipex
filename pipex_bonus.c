...
void	mid_child(char *argv[], char **env, int **tubes_arr, int i, int n_command)
{
	...
	dprintf(2, "|---mid %s IN:%d OUT:%d----|\n", argv[n_command], tubes_arr[i -1][1], tubes_arr[i][0]);
	...
	// fd_temp = open("outfile2", O_CREAT | O_WRONLY | O_TRUNC, 0777);
	// if (fd_temp == -1)
	// {
	// 	exit(EXIT_FAILURE);
	// }
	// if (dup2(fd_temp, STDOUT_FILENO) == -1)
	// 	dup2_fail(tubes_arr[i], fd_temp, "Dup2 tube_out failed, last child\n");
	// close(fd_temp);	
...
int	main(int argc, char *argv[], char **env)
{
	...
	if (fork2 > 0)
	{
		wait(NULL); //falta funciion free para cerrar todo el array de pipes en el padre
